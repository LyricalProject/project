...
$stid = oci_parse($conn, 'begin :cursor := F_Function_to_kayrim(:p1); end;');
...
if(isset($_POST['subToDel'])){
    ...
    $sql2="SELECT * FROM KAYRIMDILIK WHERE ID2 = '$key'";
    $stid3 = oci_parse($conn,$sql2);
    oci_execute($stid3);


    if(oci_num_rows($stid3)==0){
        $del = "begin del_dary_kayrimdilik.oshiru99(:zbai1);
        end;";
...
">
		...
		<form  action=" " method="POST" enctype="multipart/form-data">
			...
			<div class="all2" style="width:100%;height:100vh">
				...
				<div class="types" style="width:100%">
					<a href="../hah.php" class="hah" style="font-size:1.7em;width:100%;color:black;" >Мои объявления</a>
      ...
      ?>
                            <tr>
                                <td><?php  echo ( $cursor[$x]['NAME_K'] ) ?></td>
                              <td><?php  echo( $cursor[$x]['SROK_K'] ) ?></td>
                              <td><?php  echo ($cursor[$x]['NUMBER_YOUR'] ) ?></td>
                             <td> 
